var db = openDatabase('chinookdb', '1.0', 'Test DB', 4*1024*1024);
...
scriptsDropTables.forEach((drop)=>{
    db.transaction((tran) => {
        tran.executeSql(drop); 
     });
});
...
scriptsCreateTables.forEach((table)=>{
    db.transaction((tran) => {
...
scriptsForeingKey.forEach((foreing)=>{
    db.transaction((tran) => {
        tran.executeSql(foreing); 
     });
});
